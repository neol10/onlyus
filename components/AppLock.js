...
export default function AppLock({ children }) {
  ...
  const handleBiometricAuth = async () => {
    try {
      ...
      // Configuração otimizada para FaceID/TouchID/Digital
      const options = {
        publicKey: {
          challenge,
          timeout: 60000,
          userVerification: "required",
          // Permite que o navegador use qualquer credencial disponível no dispositivo
          allowCredentials: [] 
        }
      }
      ...
      <AnimatePresence>
        {isLocked && (
          ...
          >
            {settings.pinPhoto && (
              <div 
                className="absolute inset-0 z-0 opacity-25 blur-lg bg-cover bg-center scale-110"
            ...
            <div className="relative z-10 w-full max-w-[340px] px-8 text-center">
              ...
              <h2 className="text-xl font-bold text-white mb-1">Cofre Protegido</h2>
              <p className="text-slate-400 text-xs mb-8">Digite o PIN ou use a biometria</p>
              ...
              {/* Teclado com Layout de iPhone (Espaçado) */}
